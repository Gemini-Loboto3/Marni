 ...
 */
char new_output_fp[1024];
#define TRACE_DEBUG		OutputDebugStringA(new_output_fp)
...
static bool print_position_from_addr(const void* addr)
{
	...
	if (addr == last_addr)
	{
		if (last_info[0] == '\0')
			return false;
		sprintf(new_output_fp, "%s", last_info);
		TRACE_DEBUG;
		return true;
	}
	if (new_progname)
	{
		...
		if (fp)
		{
			...
			if (res == 0 && len > 0)
			{
				...
				{
					sprintf(new_output_fp, "%s", buffer);
					TRACE_DEBUG;
...
static void print_position(const void* ptr, int line)
{
	...
	{
		sprintf(new_output_fp, "%s:%d", (const char*)ptr, line);
		TRACE_DEBUG;
	}
	else if (ptr != NULL)   // Is caller address present?
	{
		...
		{
			sprintf(new_output_fp, "%p", ptr);
			TRACE_DEBUG;
		}
	}
	else					// No information is present
	{
		sprintf(new_output_fp, "<Unknown>");
		TRACE_DEBUG;
	}
}
...
static void free_pointer(void* pointer, void* addr, bool is_array)
{
	...
	if (ptr->magic != MAGIC)
	{
		{
			fast_mutex_autolock lock(new_output_lock);
			sprintf(new_output_fp, "delete%s: invalid pointer %p (",
					is_array ? "[]" : "", pointer);
			TRACE_DEBUG;
		...
		OutputDebugStringA(new_output_fp);
		_DEBUG_NEW_ERROR_ACTION;
	}
...
int check_leaks()
{
	...
	while (ptr != &new_ptr_list)
	{
		...
		TRACE_DEBUG;
		// dump a few bytes
		sprintf(new_output_fp,"Data: <");
		char hex[4]={0};
		for(int i=0; i<16 && i<(int)ptr->size; i++)
		{
			sprintf(hex,"%c",pointer[i]>=0x20 ? pointer[i] : '.');
			strcat(new_output_fp,hex);
		}
		strcat(new_output_fp,"> ");
		for(int i=0; i<16 && i<(int)ptr->size; i++)
		{
			sprintf(hex,"%.2X ",(unsigned char)pointer[i]&0xFF);
			strcat(new_output_fp,hex);
		}
		strcat(new_output_fp,"\n");
		TRACE_DEBUG;
	...
	{
		sprintf(new_output_fp, "*** %d leaks found\n", leak_cnt);
		TRACE_DEBUG;
	}
	return leak_cnt;
}
...
int check_mem_corruption()
{
	...
	fast_mutex_autolock lock_ptr(new_ptr_lock);
	fast_mutex_autolock lock_output(new_output_lock);
	sprintf(new_output_fp, "*** Checking for memory corruption: START\n");
	TRACE_DEBUG;
	...
	sprintf(new_output_fp, "*** Checking for memory corruption: %d FOUND\n",
			corrupt_cnt);
	TRACE_DEBUG;
	return corrupt_cnt;
}
