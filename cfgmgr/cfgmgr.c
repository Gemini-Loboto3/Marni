...
void GetDefaultConfig(DXGLCFG *cfg)
{
	...
	cfg->DPIScale = 1;
	cfg->AddModes = 0;
	cfg->CustomResolutionX = 1024;
	cfg->CustomResolutionY = 768;
...
void GetCurrentConfig(DXGLCFG *cfg, BOOL initial, int DPI_mode)
{
	//HKEY hKey;
	//Sha256Context sha_context;
	//SHA256_HASH sha256;
	...
	*/
	...
	// this works like a charm for RE1
	if (initial)
	{
		memset(cfg, 0, sizeof(*cfg));
		cfg->CustomResolutionX = cfg->WindowWidth = 640;
		cfg->CustomResolutionY = cfg->WindowHeight = 480;
		cfg->DisplayMultiplierX = cfg->primaryscalex = 1.0f;
		cfg->DisplayMultiplierY = cfg->primaryscaley = 1.0f;
		//cfg->DPIScale = 1;
		cfg->DPIScale = DPI_mode;
		cfg->WindowPosition = 1;
		cfg->RememberWindowPosition = 1;
		cfg->fullmode = 1;	// 1 and 5 working
	}
